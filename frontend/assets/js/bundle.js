...
form.addEventListener("submit", function (e) {
    ...
    if (checkFormStatus(this))
        window.alert("REGISTER SUCESSFULL ✔✔✔");
});
function checkEmptyFields(...inputs) {
    inputs.forEach((i) => !i.value ? showErrorMessage(i, "This field cant by empty") : "");
}
...
function hideErrorMessages(form) {
    form.querySelectorAll("." + SHOW_ERROR_MESSAGE).forEach((i) => i.classList.remove(SHOW_ERROR_MESSAGE));
}
...
function checkFormStatus(form) {
    let status = true;
    form.querySelectorAll("." + SHOW_ERROR_MESSAGE).forEach(() => (status = false));
    return status;
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
__webpack_require__(/*! ./form-control */ "./src/exercise/form-control.ts");
